...
Graph.Renderer.Raphael = function(element, graph, width, height) {
    ...
    this.dragger = function (e) {
        ...
        this.dy = e.clientY;
        selfRef.isDrag = this;
        //Keep original opacity so it can be restored.
        selfRef.isDrag.originalFillOpacity=this.attrs['fill-opacity'];
    ...
    document.onmouseup = function () {
      var fillOpacity=(selfRef.isDrag.originalFillOpacity?selfRef.isDrag.originalFillOpacity:0);
        selfRef.isDrag && selfRef.isDrag.animate({"fill-opacity": fillOpacity}, 500);
...
Graph.Renderer.Raphael.prototype = {
    ...
    drawNode: function (node) {
        ...
        if (node.content.getShape) {
            ...
            var ellipseSize = (node.content.size ? node.content.size : 20);
            //shape = this.r.ellipse(point[0], point[1], ellipseSize * 1.5, ellipseSize);
            shape = this.r.ellipse(point[0], point[1], 1.5 * ellipseSize, ellipseSize);
            var color = (node.content.color ? node.content.color : "black");
            var fillOpacity=(node.content.fillOpacity ? node.content.fillOpacity : 0)
            shape.attr({ fill: color, stroke: color, "fill-opacity": fillOpacity, "stroke-width": 2 })
        }
        shape.mousedown(this.dragger);
        if (node.content.href) {
          shape.click(function(e) {if (e.ctrlKey) window.location.href = node.content.href;;});
        }
        if (node.content.mouseover) {
          shape.mouseover(node.content.mouseover);
        }
        if (node.content.mouseup) {
          shape.mouseup(node.content.mouseup);
        }
